...
import type { IUserNoPassDocument } from '@/models/user.model.js';

type UserVerifiedPayload = JwtPayload & IUserNoPassDocument;

const extractUser = (
  decodedPayload: UserVerifiedPayload
): IUserNoPassDocument => {
  const { iat, exp, nbf, jti, aud, iss, sub, ...userPayload } = decodedPayload;
  return userPayload;
};

export const generateAccessToken = (payload: IUserNoPassDocument): string => {
  return jwt.sign(payload, ACC_JWT_CONFIG.secret, ACC_JWT_CONFIG.options);
};

export const generateRefreshToken = (payload: IUserNoPassDocument): string => {
